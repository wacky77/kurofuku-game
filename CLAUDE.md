...
**v56追記:** 更新時は `index.html` の `?v=N`、`js/assets.js` の `ASSET_V`、`sw.js` の `CACHE` / `MEDIA_CACHE` / `MEDIA_V` / `ASSETS` を同じ版数へ更新する。MEDIA_CACHEは配信用画像・音声を事前保存し、activate時に旧世代を削除する。v56では指名客の振り返り・パーフェクト実績判定も指名本人基準へ統一した。以下のv55時点の記録より、この追記を優先する。
...
- キャッシュ: 音声URLは `?v=ASSET_V` 付き。sw.jsは画像・音声を世代付き`MEDIA_CACHE`へ事前保存し、更新時に旧世代を削除する。**曲を差し替えたら ASSET_V / MEDIA_V と各キャッシュ版数を揃えて上げる**（画像も再取得になる点に注意）。
