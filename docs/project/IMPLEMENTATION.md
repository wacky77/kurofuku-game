# 付け回しマスター — 実装アーカイブ（旧CLAUDE.md 完全版）

> このファイルは旧 `CLAUDE.md` の完全版アーカイブ。運用の正はルート `CLAUDE.md`（軽量版）。
> 過去の全変更履歴・実装詳細が必要なときだけこのファイルを読む。以下、旧CLAUDE.md全文。
...
- ファイル:
  ...
  - `js/achieve.js` — 実績（アチーブメント19種＝通常14＋隠し5、localStorage永続化＋解除トースト）
...
**v56追記（更新: sw.jsのキャッシュ自動生成化）:** アセット更新時は **`index.html` の `?v=N` と `js/assets.js` の `ASSET_V` の2箇所だけ**を揃えて上げる。`sw.js` は冒頭で `importScripts('./js/assets.js')` し、`CACHE`/`MEDIA_CACHE` を `ASSET_V` から自動導出、事前キャッシュ対象URLも `ASSET_IMG`/`ASSET_AUDIO` マニフェストから自動生成する（手書きのID一覧は廃止＝二重管理・更新忘れによる install 失敗事故を解消）。画像・音声を追加したら `js/assets.js` のマニフェストに足すだけで事前キャッシュに含まれる。背景画像の版数は `js/assets.js` の `BG_V`（差し替え時のみ上げる。据え置きなら他の更新と無関係）。MEDIA_CACHE（画像・音声）は1件ずつ取得を試み、失敗しても install 全体は止めない（失敗許容。欠けた分はオンライン時の実行時キャッシュで自己修復）。アプリシェル（CACHE、コアJS/CSS）は従来どおり addAll で全件成功必須。v56では指名客の振り返り・パーフェクト実績判定も指名本人基準へ統一した。以下のv55時点の記録より、この追記を優先する。
`index.html` の `<script>`/`<link>` は `?v=N` 付き。**アセット更新時は index.html の `?v=N`・`sw.js` の `CACHE`/`ASSETS`・`js/assets.js` の `ASSET_V` の版数を揃えて上げる**（でないと旧版が配信される）。**現在 v59**（v28: BGM外部音源化／v29: BGM曲別音量／v30: 新人特性〈伸びしろ〉／v31: 客の見極め・来店演出・振り返り強化／v32: 高単価ナーフ＝イベント重み付け＋目標再調整／v33: UI整理＝文字削減・レイアウト圧縮／v34: 交代の後任を候補2名から選択／v35: 交代画面に「残留」選択肢／v36: 交代を枠の紐付けなしのプール選択に＝現メンバーN人＋候補2N人から残るN人を自由に組み合わせ／v37: ふりかえりに指名バッジ＝💐本指名（指名で来店）・🎀場内指名（★4以上でその場獲得）／v38: 新キャスト「つかさ」追加＝ギャル+語尾「ござる」の新voiceタイプ gal_gozaru／v39: 交代を1画面に統合（swappick画面廃止）＝現メンバーカードのタップで交代指定⇄残留トグル、下段に常時表示の候補プールから空き枠分だけ選択／v40: 接客結果を★の数で5段階に演出差別化＝★0-1は暗め・光らず地味、★2-3は標準、★4は金枠グロー、★5は強い金色グロー+脈動+入場シェイク+キラキラパーティクルで特大成功を演出／v41: タイムリミットに円環ゲージを追加＝残り時間が視覚的に減っていくSVGリング表示、残り3秒以下でリングも赤化／v42: タイトルロゴをCSSだけで金色豪華スタイルに強化＝グラデーション+エンボス影+装飾ライン、画像アセットなし／v43: タイトルロゴを実画像化＝ユーザー提供の透過金色ロゴ画像(assets/images/logo.png)をタイトル画面に表示（v42のCSSテキストロゴはフォールバックとして保持）／v44: タイトルロゴ画像を再アップロード版に差し替え＝今度は正規の透過PNG（チェッカー柄の焼き込みなし）だったためトリミング・圧縮のみで反映／v45: タイトルロゴの光沢エフェクトを差し替え＝斜めの矩形光沢バーが走る `logoShine`（ユーザー指摘「四角で違和感がある」）を撤去し、`.nc-spark`と同系統のキラキラ星（`.logo-spark`、ロゴ周囲にランダム配置で明滅する`logoTwinkle`）に変更。ロゴ自体の画像は変更なし）／v46: 「本音を隠す客」のヒントをキャラ立てて強化＝ユーザー指摘「同じニーズなら医者もお笑い芸人も同じ4文の使い回しで厚みがない」。`CUSTOMER_TYPES`28人ぶんに`hints`（heal/talk/price/smile各1文・計112文、職業ペルソナに寄せた本音ヒント）を追加し`makeCustomer()`で優先使用（無ければ`NEEDS[].hints`にフォールバック）。表示も「🤔 本音を察して…」というメタなラベル文言を撤去し、ヒント文中の決め手の語句（`**語句**`マーカー→`formatHint()`が`<b class="hint-key hint-{need}">`に変換）だけをニーズ別4色（heal/talk/price/smile）で強調する表現に変更。背景画像は CSS(`#phone[data-bg]`) から参照するので、背景差し替え時は `css/style.css` の url(...) 版数も忘れず揃える。／v47: v46のヒントをユーザー指摘3点で全面書き直し＝①「」会話文だと"接客後に喋った"体になり初対面感がない、②ニーズのラベル文言（癒されたい／盛り上がりたい等）をほぼそのまま使っていた文が混在、③文章が長く時間制限のあるゲーム中に読みにくい、の3点。`CUSTOMER_TYPES`112文＋`NEEDS[].hints`16文＝計128文すべてを、セリフを一切使わず**入店した瞬間・見た目だけで分かる情報**（服装・小物・表情・姿勢・肌ツヤ等）に限定した8〜16文字程度の短文に差し替え。ラベル文言・直接的な言い換えは排除。職業キャラ性は聴診器・和帽子・万年筆等の持ち物描写で表現。`.cust-need.vague`（css/style.css）はセリフでなく地の文の観察描写になったため非イタリックに変更。／v48: 「本音を隠す客」の出現率を引き上げ＝ユーザー指摘「客のコメント（観察ヒント）を書き換えたのに一部の客は変わって見えない」。原因は仕様どおりで、ヒントは`vague`（本音を隠す客）のときだけ表示され、それ以外は`「癒されたい」`とニーズを直接表示するため、v46/v47で書き換えたヒントが出る頻度が低かった。`DAY_CONFIG.vagueChance` を 0.25→**0.40**（DAY1基準）、`vagueChanceMax` を 0.6→**0.75** に引き上げ（`vagueChancePerDay 0.04` は据え置き）。judge/chooseCastロジックは不変＝simへの影響なし（表示頻度のみの変更）。／v49: UI改善6件（見やすさ＋ジュース演出。ロジック不変＝sim影響なし）＝①ニーズ4色統一: `:root`に`--need-{heal|talk|price|smile}`を定義し、ヒント強調（.hint-*）・通常客のニーズ表示（.cust-need.need-*、nominationは従来のクリーム色を維持）・結果ポップの答え合わせ（.res-reveal b）・選抜カードのステータス行ドット（statRows の .ndot、.stat-label幅3.6→4.7em）・振り返りラベル（State.logに`need`追加→.rv-cust small.need-*）で共通化。②組数ピップ: HUDの「n / N組」を .hud-pips（●=済/現在=金グロー/未=灰）＋小さい数字に。イベント位置は事前に見せない。③前回★バッジ: play画面のキャストカード左上に今夜の直近★（.cc-last、State.logから逆引き、★4以上は金グロー.hi。logは日次リセット＝自動で「今夜」限定）。④売上バー演出: 結果ポップ「次のお客へ」で`flySalesToGoal()`＝「+¥N」金色フロート(.sales-fly)がWAAPIで目標バーへ吸い込まれ→バー伸長＋.flashキラッ→100%初回のみ.celebrate大glow＋SFX.vip（State.goalCelebrated、日次リセット）。バーは進捗50/80/100%で.t50/.t80/.done段階グロー。時間切れ（★0）や¥1000未満は演出スキップ。⑤コンボ段階演出: .combo-badge.c3（3連続〜脈動）/.c5（5連続〜強グロー＋.combo-vignette画面縁金）＋途切れ時`breakComboBadge()`で.break砕け演出（chooseCastの!hit時とtimeUp時）。⑥焦り演出: 残3秒で.customerに.sowasowa（transform微振動、custIn完了後にanimation差し替え）。／v50: 編成・選抜画面の視認性向上（ユーザー要望6点。表示のみ・ロジック不変）＝①ステータス行のラベル（癒し/トーク/単価/笑顔）をニーズ4色で色分け（statRowsの`.stat-label need-*`）、v49で入れた色ドット`.ndot`は廃止。②名前下のタグ（`.cast-tag`＝高単価等）とプロフィール文（`.cast-profile`）を選抜・編成カードから撤去し情報を整理（キャラ性は顔枠色＋詳細モーダルℹ️で担保）。③顔アイコンを76→92pxに拡大。④DAY表記を大きな金バッジ化（`.form-head`＋`.day-badge`、選抜画面にも`DAY ${State.day}`を追加）。⑤編成画面の「🔄交代する人をタップ…」キャプション（.head-sub）を撤去。⑥顔アイコンの縁をそのキャラの最得意ニーズ色で縁取り（`bestNeed()`→カードに`--ring`をセット、`.cast-avatar .avatar`が枠色＋淡グロー）。judge/chooseCast不変＝sim影響なし。／v51: 客ペルソナと画像の不一致を是正（ユーザー指摘「職業・年齢が絵と合わない」）＝①画像の取り違え修正: 来店客画像のうち8枚が別idに割り当てられていた（板前↔医者↔大学教授↔建設の親方↔弁護士の5者ローテーション＋部長↔バンドマン↔若手社員の3者ローテーション）。画像は高品質だったため再生成せず、`assets/images/customers/*.jpg` と `_src/*.png` のファイル名を正しいidへ入れ替え（PILでの目視コンタクトシート照合で確定）。②年齢の固定: `CUSTOMER_AGES`（20〜60代）を毎回ランダム割当していたため絵と年齢がちぐはぐだった。`CUSTOMER_AGES_BY_ID`（28ペルソナ×画像の見た目に合う年代1〜2個）を追加し `makeCustomer` が `type.id` で引く（無い場合＝イベント客等はフォールバックで従来ランダム）。画像ファイルを差し替えたため ASSET_V を51に更新（全画像・音声が再取得）。判定ロジック不変＝sim影響なし。／v54: ニーズ表示を色ベース化（本音を言葉にせず顔リング＋ムードチップの色で示す。本音を隠す客(vague)は色を伏せヒント文の決め手語句もニーズ色にしない）＋指名客・VIP等イベント客のセリフ増量（指名は相手キャストのvoiceで味付け）。／v55: 黒服ランク7段階を絵文字から**紋章画像**に（下記「画像アセット」のランク紋章を参照）。**注意: `css/style.css` の背景画像 url は `?v=23` のまま**だが、背景ファイル自体を差し替えていないため意図的に据え置いている（上げると約1MBの背景が無意味に再取得される）。次に**背景画像を差し替える時だけ**この4行の版数を上げること。／v56: sw.jsのキャッシュ自動生成化＋指名客判定の統一（上記「v56追記」参照）。／v57: **全国ランキングをFirebase（匿名認証＋Cloud Firestore）でDB化**（下記「全国ランキング」参照）。新規JS 2本（`js/firebase-config.js`・`js/cloud-scores.js`）を index.html/sw.js に追加、タイトル＆結果画面に全国ランキング表示を組込。Firebase未設定・通信失敗時は端末内ランキング（従来のlocalStorage Top5）へ自動フォールバック＝ロジック・sim影響なし。／v58: **生涯キャリア＋隠し実績（周回要素の第1弾）**＝runをまたぐ累計成績 `kurofuku_career` の永続化・タイトル常設のキャリアバッジ（`CAREER_RANKS`＝RANKSの上位3段延長）・隠し実績5種（`hidden: true`、未解除は「？？？」表示）。詳細は下記「実装済み（主要）」の生涯キャリア項を参照。判定ロジック不変＝sim影響なし。／v59: **デイリーチャレンジ「📅 本日の営業」（周回要素の第2弾）**＝日付シード（yyyymmdd）で応募8人・来店順を全員共通にしたDAY3相当固定難度の1日スコアタ＋日付別の全国ランキング（Firestore `daily/{yyyymmdd}/scores/{uid}`）。詳細は下記「実装済み（主要）」のデイリーチャレンジ項を参照。通常モードは `RNG = Math.random` のまま＝判定ロジック・sim影響なし（sim実測で従来値を確認済み）。**注意: `firestore.rules` にデイリー用ルールを追記済みだが、Firestoreコンソールへの貼り付け（公開）はユーザー操作待ち**＝未公開の間、デイリーの全国ランキングは「準備中」表示・記録は端末内のみ（グレースフルにフォールバック）。

## 全国ランキング（Firebase / v57）
- **構成**: Firebase Authentication（匿名認証）＋ Cloud Firestore。プロジェクトID `kurofuku-game`（ロケーション自動／Standardエディション）。SDKは `js/cloud-scores.js` が gstatic CDN から動的 `import()` で読み込む（npm/ビルド不要のまま）。
- **接続情報**: `js/firebase-config.js` の `window.KUROFUKU_FIREBASE_CONFIG`（apiKey等。**Webクライアント公開前提の接続情報＝秘密鍵ではない**。data保護は下記ルールと認証で担保）。null なら全機能が端末内ランキングにフォールバック。
- **データモデル**: `scores/{uid}` に1端末1レコード（匿名uidがドキュメントID）。フィールド `uid/name(1-8字)/sales(0-999999999)/day(1-999)/createdAt(serverTimestamp)`。`submitScore` は自己ベスト（sales→day順）超過時のみ setDoc（`updated:false/true` を返す）。`topScores` は sales desc で多めに取り、同額はクライアントで day 降順に整列して上位N件。
- **セキュリティ**: `firestore.rules`（read全許可・create/updateは本人uid＆値域検証＆自己ベスト超過のみ・delete禁止）。**このルールをFirestoreの「ルール」タブに貼って公開する運用**（デプロイ自動化はしていない）。
- **セットアップ手順**: `FIREBASE_SETUP.md`（プロジェクト作成→Webアプリ登録→匿名認証ON→Firestore作成→ルール公開→承認済みドメインに `wacky77.github.io` 追加）。
- **検証済み（2026-07-11）**: ローカル（localhostは自動で認証許可）で topScores/submitScore の往復・下位無視・上位更新まで確認。テスト投入したレコードはコンソールから削除する（ルールでクライアント削除は不可）。
- **デイリーチャレンジ用（v59）**: `daily/{yyyymmdd}/scores/{uid}` に1端末1レコード（フィールド `uid/name(1-8字)/sales/date(=パスの日付)/createdAt`、自己ベスト超過時のみ更新可）。`js/cloud-scores.js` の `topDailyScores(date, max)` / `submitDailyScore({name, sales, date})`。通常の `scores/{uid}` とは完全独立。**ルールは `firestore.rules` に追記済み・コンソールへの貼り付けが必要**（未公開の間は permission-denied → クライアントは「準備中」表示と端末内ベストにフォールバック。実機で確認済み）。
...
- **実績19種＝通常14＋隠し5**（js/achieve.js）: 通常14＝DAY到達系/★5/パーフェクト/コンボ6/指名/太客/リピーター10/通算100万・500万/Lv5/店長昇格。
  隠し5（`hidden: true`・v58）＝介抱のプロ（酔っ払いに★5）/迷いなき采配（時間切れ0でDAY5達成）/フレッシュ開店（全員新人の編成で目標達成）/
  生きる伝説（伝説の黒服に昇格）/夜に生きる男（生涯売上3000万）。未解除の隠しは一覧で「？？？（隠し実績）」表示（showAchievements）。
  解除は `Achieve.onServe`（chooseCast内）と `Achieve.onDayEnd`（endDay内）のフックで判定→金色トーストをキュー表示。
  タイトルの「🏅 実績 n/19」ボタン→ `showAchievements()` で一覧（未解除は🔒グレー表示）。
- **生涯キャリア（v58・周回要素）**: run をまたぐ累計成績を localStorage `kurofuku_career`（`{lifeSales, lifeServes, lifeRepeaters, runs}`）に永続化。
  加算は game.js の `addCareerDay()`（endDay 内・その日の売上/接客数/場内指名数）と `startSelection()`（runs+1、途中離脱runも1回と数える）。
  キーが無い・壊れている場合は `loadCareer()` がゼロ始まりにフォールバック＝既存ユーザー後方互換。
  タイトルに常設のキャリアバッジ（`.career-badge`、1回でも営業すると表示）＝キャリアランク＋生涯売上＋通算営業回数＋次の称号まで。
  キャリアランクは data.js の `CAREER_RANKS`（RANKS 7段の上位延長: 夜の顔役2000万/夜の帝王3500万/不夜城のオーナー6000万）＋
  `careerRankFor()`。index 0〜6 は RANKS と同一＝紋章画像を流用、7以降は `rankIcon` の emoji フォールバック。
  隠し実績用に `State.runTimeouts`（timeUp でカウント・runごとにリセット）と `Achieve.onServe` の `custId` /
  `Achieve.onDayEnd` の `lifeSales`/`runTimeouts`/`allRookie` を追加。judge/chooseCast/difficulty は不変＝sim影響なし（sim実測で従来値を確認済み）。
- **デイリーチャレンジ「📅 本日の営業」（v59・周回要素）**: タイトルの専用ボタンから、**日付シード（yyyymmdd・端末ローカル日付）で応募8人と来店順を全員共通**にした
  DAY3相当固定難度（`DAILY.day=3`→`difficulty(3)`）の1日スコアタ。**何度でも挑戦可・記録は自己ベストのみ**（1日1回制限はクライアントで強制できず、
  同条件リトライこそが本モードの面白さのため）。実装は game.js の `RNG` 間接参照（`let RNG = Math.random`＋`mulberry32`＋`withSeededRNG()`。
  生成系 rand/randWeighted/pickRandom/makeCustomer/buildDayCustomers が RNG() を使用）で、応募抽選=シード値・来店生成=シード値+1 の2ストリーム。
  **judge() の★ブレは常に Math.random**＝状況は同じ・結果は実力と運。通常モードは RNG=Math.random のまま完全に従来挙動（sim確認済み）。
  フロー: `startDailySelection(dateStr?)` → 選抜（バッジ「📅 本日の営業」）→ startDay（HUD「📅 本日」）→ `endDaily()` → `renderDailyResult()`
  （自己ベスト表示・日付別全国ランキングTOP10・登録フォーム・同一シードで「もう一度挑戦」）。端末内自己ベストは localStorage `kurofuku_daily`
  （`{date, sales}`・日付が変わると自動無効）。生涯キャリア（runs/lifeSales等）にはデイリーも加算。resetToTitle が mode を normal に戻す。
...
- 置き場: `assets/images/{icon,cast,customers,backgrounds,rank}/`（ファイル名＝id）
- **ランク紋章（v55）**: `assets/images/rank/1.png`〜`7.png`（**ファイル名＝`RANKS` の添字+1**）。透過PNG・256px・パレット256色、
  1枚18〜27KB／7枚計155KB。`js/assets.js` の `ASSET_IMG.rank`（index の Set）＋ `rankIcon(index, size, fallbackEmoji, cls)` で切替＝
  画像が無い段階は従来の `<span class="rank-emoji">` にフォールバック。表示は4箇所: 昇格バナー（88px・暗いメダリオン `.rankup-medal` に載せる。
  帯の背景が明るい金なので金の紋章が沈むのを防ぐため。`medalIn` で弾んで登場＋`medalGlow` で脈動）／DAY結果のランクカード（34px）／
  タイトルの最高役職バッジ（20px `.inline`）／店長以上GAME OVERの労いカード見出し（20px `.inline`）。
  昇格判定の `State.rankUp` に `toIndex` を追加済み（`RANKS` オブジェクトだけでは添字が引けないため）。
  生成プロンプトは `asset-gen/prompts/kurofuku-rank.json`（7件・専用プリセット。`kurofuku.json` とは分離＝既存4件の無駄な再生成を防ぐ）。
  圧縮は sharp（`resize(256).png({palette:true})`）。**プロンプトに「文字・ロゴ・banner のリボン文字は一切描かない。」を必ず入れる**
  （初回生成で副店長・エリアマネージャーに "ASSISTANT MANAGER" 等の英字が焼き込まれ、2枚を再生成した経緯。ボツ原本は `rank/_src/_rejected/`）。
- **現状**: 背景4枚・アイコン3枚・**キャスト全28人**・**来店客全32種**・ロゴ1枚・**ランク紋章7枚**、すべて画像化済み＝**全て圧縮済み**（合計約5.1MB）。SVG似顔絵は完全フォールバック扱い。
...
- キャッシュ: 音声URLは `?v=ASSET_V` 付き。sw.jsは `js/assets.js` の `ASSET_AUDIO`（bgm/sfx）マニフェストから事前キャッシュ対象を自動生成し、世代付き`MEDIA_CACHE`へ保存、更新時に旧世代を削除する。**曲を差し替えたら ASSET_V を上げる**（画像も再取得になる点に注意）。新規BGM/SEを追加した場合は `ASSET_AUDIO` にidを足すだけで事前キャッシュに含まれる。
...
## 残課題
- **デイリーチャレンジのFirestoreルール公開（ユーザー操作）**: `firestore.rules`（v59でデイリー用を追記済み）を
  Firebaseコンソール → Firestore →「ルール」タブに全文貼り付けて「公開」する。公開までデイリーの全国ランキングは
  「準備中」表示・記録は端末内のみ（グレースフル動作は実機確認済み）。
（2026-07-11: 周回要素第2弾＝デイリーチャレンジ（v59）・第1弾＝生涯キャリア＋隠し実績5種（v58）・全国ランキングのFirebase DB化（v57）を実装・検証・完了）
次にやるなら（周回要素の続き・提案済みの順）: 名鑑（キャスト/客コレクション）、全国ランキングへの称号表示（Firestoreフィールド追加）、キャストのボイス的なセリフ演出、全国ランキングにApp Check導入（機械投稿抑制・FIREBASE_SETUP.md「任意の追加対策」参照） 等。
